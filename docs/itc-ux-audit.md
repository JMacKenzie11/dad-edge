...
#### [Resolved 2026-08-31] `worry.coach_commitment_draft` doesn't refresh when a worry is edited

- **What was flagged:** commitment drafts landed on advance but didn't refresh after a worry sharpen; coachee saw a draft speaking to the old worry.
- **How it was resolved:** the whole drafts-and-accept path for commitments was retired. Commitments now auto-derive straight to `itc_commitments.text` on advance (via `autoDeriveCommitmentsAfterAdvance`) and re-derive on every worry edit (via `autoDeriveCommitmentForWorry` in `saveWorry`). Migration `20260831000009_itc_drop_coach_commitment_draft.sql` drops the `coach_commitment_draft` column. Coachees can still edit commitments inline; the derived text is the starting point.

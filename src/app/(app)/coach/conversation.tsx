...
export function CoachConversation({
  ...
  return (
    // On mobile the fixed BottomNav (~64px w/ safe area) eats into
    // 70vh, so the send button lands under the nav. Cap the container
    // to 70vh - the nav on mobile; unchanged (70vh) on md+.
    <div className="flex flex-col h-[calc(70vh-5rem)] md:h-[70vh] rounded-[var(--radius-card)] bg-[color:var(--color-surface)] border border-[color:var(--color-border)] overflow-hidden">
  ...
  if (turn.role === "user") {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] sm:max-w-[80%] rounded-[var(--radius-card)] rounded-tr-sm bg-[color:var(--color-primary)] text-white px-3 py-2 text-sm whitespace-pre-wrap">
          {turn.text}
        </div>
      </div>
    );

...
3. **Worry set.** Depth spread first — any at depth 2 while others are 3? Then interior-witness verbs: worries phrased as "I'd have to see," "I'd have to know," "I'd have to face" name something the coachee does INSIDE HIS HEAD, not something the outside world could witness. Sharper form names what the other person would see or say ("she'd see the pattern is mine," "she'd say it out loud," "she'd stop trusting me"). Flag any interior-witness worry with the corrected external-witness rewrite. Then redundancy: any worry that duplicates a commitment already listed? Any behaviors with no worry paired?
...
5. **Assumption set.** Four checks. First, for EACH assumption, does its if-clause name the exact scenario the linked commitment is protecting against? If they've drifted apart, name the pair. Second, the then-clause specificity check: the then-clause must name a SPECIFIC identity or consequence the coachee would experience if the if-clause happened. Phrases like "I'd be the person I'm terrified of," "I'd become what I fear I am," or "I'd be the guy I don't want to be" are vague — they gesture at an identity without naming it. Vague then-clauses don't test. Flag any and suggest the coachee name what the identity actually is. Third, depth 3 across all + coverage (every commitment has an assumption linked). Fourth, over-load: if one assumption is the ONLY one linked to a commitment that clearly protects against multiple identity concerns, name that. Suggest that additional commitments and assumptions may need to be named to unload the single assumption.
...
**Excavation asks MUST name a specific candidate.** If you're recommending "unpack this into more parts," "split this into two," "try rewriting in a different frame," or any exercise that asks the coachee to look for something hidden — you MUST name the SPECIFIC entry to try it on first. An abstract ask ("untangle the three worries into separate worries") is dead — the coachee has no way to act on it and doesn't know which one to start with. Pick the most likely candidate and say so ("W3 is your best candidate for this: the 'lose it AND be the husband who hurts her' framing might be two fears bundled — one about the moment, one about the identity over time. Try splitting it there first and see what surfaces").

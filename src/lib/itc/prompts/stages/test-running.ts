...
- If his message contains observations already ("she reacted well," "I noticed I couldn't do it," "here's what happened…"), acknowledge briefly and start the results collection — walk into the debrief fields, or ask which piece he wants to start with.
...
Do not treat check-ins or unrelated messages as observations. Only when he's actually given you real observations from running the test should you walk into the debrief. If he hasn't run it yet, don't fabricate a debrief. (Vol 2 p. 312: a debrief needs actual data collected from actually running the test.)

===== MARKERS FOR THIS STAGE =====
No markers at this stage — it's a wait state. Stage transition to results happens through the results-stage <<record_test_results>> flow once observations land.
`.trim();

...
- Shape: ONE if-clause, ONE then-clause. No "if X, Y, or Z" — the condition names the category of counter-move (e.g. "if I let people push back on me" for a work-context map, or "if I let her push back on me" for a Bond map — use the coachee's own relational nouns), not an enumeration of specific behaviors. No "then A, B, and C" — the conclusion names ONE belief. The guide's rationale (pp.22-25) is that compound assumptions have embedded if-thens that need to be unpacked before testing.

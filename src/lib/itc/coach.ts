...
// The Column 3 worry drafter lived here: draftWorryForBehavior and
// draftWorryOutcome, ~250 lines. Deleted 2026-09-03 when its last
// caller went.
//
// It authored the whole sentence, counter-move and fear together.
// The counter-move is a mechanical inversion of his own behavior and
// the server was right about it every time; the fear is his, and on
// live maps the identity landing was wrong about one time in three.
// Tuning the judges never moved that, because the question
// underneath ("does this habit protect him from this fear, or cause
// it?") needs what is true inside one man's head, and both readings
// are constructible from the text every time. Measured: the judge
// scored 4/6 on fresh cases, missing only in the permissive
// direction; the same question asked in isolation scored 0/4, on the
// main model as well as the utility one.
//
// What replaced it: draftWorryOpening writes "I worry that if I
// <counter-move>, " and stops. Kegan Vol 1 p 12 has the coach ASK
// "what are the worst things that could happen if you did the
// opposite?" rather than answer it. Where a worry needs work the
// coach names what is missing and the man rewrites it himself.

...
_Snapshot of current app behavior as of 2026-09-01. Reverse-engineered from the codebase at `/Users/jasonmackenzie/Custom Applications/Dad Edge Brave Man OS/`._
...
- **Competing commitments auto-derive from worries 2026-08-31:** Column 4 stops being a drafts-and-accept surface. On advance from worries → commitments, the server runs `draftCommitmentForWorry` per worry and writes the non-noble transformation straight to `itc_commitments.text` (via `upsertCommitmentForWorry` + `scoreCommitmentDepth` + `composeCommitmentSharpen` — same pipeline as `saveCommitment`, so depth pill + sharpen box render from the start). Any worry edit thereafter re-fires `autoDeriveCommitmentForWorry` in `saveWorry`, keeping the paired commitment coherent with the current worry text. Coachees still edit commitments inline — the auto-derive is the starting point, not a lock. Migration `20260831000009_itc_drop_coach_commitment_draft.sql` drops the `itc_worries.coach_commitment_draft` column. Deleted: `setWorryCommitmentDraft`, `clearCommitmentDraftsForMap` (in `maps.ts`); `regenerateCommitmentDrafts`, `redriveCommitmentFromWorry` server actions; the "Use this draft" card + `StaleUpstreamBanner` "RE-DERIVE" button in `commitments-row.tsx`; the `"commitments"` mode on `RegenerateDraftsButton` (only "worries" and "assumptions" remain). Regression tests `m` (advance derives real commitments now, not drafts) and `r` (structural lock: no draft UI, saveWorry must call auto-derive) rewritten accordingly.
- **Worry redundancy check renamed + reframed 2026-08-31:** `checkWorryCommitmentRedundancy` retired; new `checkWorryRedundancy` in `src/lib/itc/criteria/worries.ts` runs pairwise worry-vs-worry (order-independent, finding on the later worry with `relatedEntryRef` pointing at the earlier one). Issue type `worry_commitment_redundancy` → `worry_redundancy`. Reason: since the auto-derive change, competing commitments mirror their paired worry by construction, so the old worry-vs-non-paired-commitment check was worry-vs-worry redundancy in disguise, but rendered as the category error "worry duplicates commitment" (a coachee-facing "how can a worry duplicate a commitment?" moment). Renderer clauses updated to say "Duplicates the worry X" instead of "Duplicates the commitment X". `runWorriesCriteria` in the orchestrator no longer needs commitments passed in.
- **Assumption drafts regenerate always visible 2026-08-31:** consolidated the `RegenerateDraftsButton kind="assumptions"` to ONE persistent location at the bottom of the assumptions column (below `AddAssumptionForm`). Previously rendered in two conditional locations — the drafts header (only when drafts existed) and an empty-state area (only when neither drafts nor accepted assumptions existed) — which meant during honing (after accepted assumptions existed) the button vanished and coachees couldn't ask for fresh drafts against the updated map. `regenerateAssumptionDrafts` action already handled the "accepted assumptions exist" case correctly (bypasses the `existingAssumptions` guard, wipes drafts, re-drafts against current commitments, leaves accepted rows untouched); only the UI gate was blocking access. `canAskForDrafts` dead-code const removed.
- **Commitment drafter verify loop matches the hone auditor 2026-08-31:** `draftCommitmentForWorry` previously verified only against the depth rubric (`scoreCommitmentDepth`), while the hone auditor also runs `checkInteriorWitnessInCommitments` (regex) and `checkCommitmentMirrorsWorry` (reads `mirrors_worry_identity`). Drafts could clear depth but fail the other checks — the coach then contradicted itself: the system auto-derived a commitment on advance, then the hone audit told the coachee that same commitment needed sharpening. Extended `verifyDraft` inside `draftCommitmentForWorry` to run all three checks (depth score, `mirrors_worry_identity` boolean from the depth call, and the interior-witness regex). Retry prompt combines every failed check's feedback into one pass so the LLM fixes them together. Best-of-two persist on retry-still-fails preserves the "never silent drop" invariant. Prompt also gained an explicit "if I did X, she'd know I'm Y" anti-pattern section calling out that the vow mirrors the CONSEQUENT identity, not the antecedent behavior — the exact drift that produced "I'm also committed to never admitting I'm wrong" from a worry about "being the kind of guy who is incapable of changing". Regression `m` extended: auto-derived commitments must also clear the interior-witness regex (drafter and auditor bar in sync). Regression `r`(d) locks structural: the drafter's verify block must reference both `checkInteriorWitnessInCommitments` and `mirrors_worry_identity`.
- **Worries drafter verify loop matches the hone auditor 2026-09-01:** same alignment fix as commitments, one column upstream. `draftWorryForBehavior` previously verified against depth (`scoreWorryDepth`) + `checkWorryLogicalConsistency` (drafter-specific inverted-identity slot check); it did NOT run `checkInteriorWitnessInWorries` (regex the hone auditor uses). So the drafter could produce "I'd have to see I…" text that passed depth but the auditor later flagged as interior-witness. Extended `verifyDraft` to include the auditor's regex; retry prompt combines every failing reason into one rewrite pass. Fail-open on depth-rubric error unchanged; deterministic checks (consistency + interior-witness) never throw. Regression `r`(e) locks structural: the worries drafter's verify block must reference `checkInteriorWitnessInWorries`.
- **Observation-severity retired 2026-09-01:** severity union collapsed from `critical | moderate | observation` to `critical | moderate`. Observations (nice-to-have signals like "worth noting" — the only source was `checkWorryRedundancy`) were too strict for a self-service coach that should only speak up when something structural or shape-level actually needs sharpening. Deleted: `Severity: "observation"` from `types.ts` + `SEVERITY_ORDER.observation`; `IssueType: "worry_redundancy"`; the entire `checkWorryRedundancy` function (was an LLM call per worry pair — pure token waste once we're not surfacing its output); its call site in `runWorriesCriteria`; `renderRedundancyClause` + `renderMergedRedundancyClause` + the "smaller observation" opening copy in `render.ts`. Now enforced at the type level — TS will reject any future attempt to author an observation-severity finding. Tests: the two render tests covering redundancy clauses removed; the "every issue type × every severity renders" exhaustiveness test updated to only iterate over `critical | moderate`; test naming updated.
- **Worry-depth rubric tightened + one-shot backfill 2026-09-01:** the `touches_identity` criterion in `scoreWorryDepth` was calibrated too loosely — it was passing worries that named FAILURE EVENTS or OUTCOMES ("I'll have failed to help them", "I'll miss an opportunity to demonstrate my value", "I won't deliver the level of value I'm capable of") as identity-level. The auditor reads the stored score, so these all slipped through hone with `depth_score=3`. Rewrote the criterion to require an EXPLICITLY NAMED identity noun in the worry text itself (self-label like "I'm a fraud" OR named role like "the coach who talks a great game but can't deliver"). Added a strict-reject list covering the exact patterns that were leaking through, plus a "could you finish 'the identity he's afraid of being is: ___' using words FROM the worry" self-check. New Node script `scripts/rescore-itc-worries.ts` (npm script `itc:rescore-worries`) re-scores every existing `itc_worries` row against the new bar; only writes when `(depth_score, rubric_reason)` actually change (so no-op re-scores don't trigger the map-level stale flags falsely). Dry run by default, `--apply` to persist. Idempotent.
- **Regenerate-drafts button reports draft count 2026-09-01:** `regenerateAssumptionDrafts` + `regenerateWorryDrafts` return type widened from `ActionResult` to `{ ok: true; draftsWritten: number } | { ok: false; reason }`. The count is threaded from `saveAssumptionDrafts` + `draftMissingWorriesAfterAdvance` (both now return the number persisted). `RegenerateDraftsButton` renders `"N fresh draft(s) added above."` on success and `"The coach couldn't produce fresh X drafts against your current Y. Try sharpening a Z first, then regenerate again."` on the silent-zero case. Fixes the class of bug where the drafter LLM returned nothing (or every draft got filtered for missing links) and the coachee saw a spinner then no change with no feedback.
- **Replace-during-honing on assumption drafts 2026-09-01:** `DraftCard` now offers `Replace #N` per accepted assumption alongside the default `Use as new`. Clicking `Replace #N` opens a themed confirmation (destructive, shows old + new text), then chains client-side: `saveAssumption` (new) → `removeAssumption` (old) → `dismissAssumptionDraft`. Save-first ordering means if the remove step fails (tests attached), the new assumption is created and the coachee gets an actionable error rather than losing data. Fills the honing UX gap where every draft acceptance was an "add" and coachees had to manually clean up their doubled column. Default action label renamed from `Use this draft` → `Use as new` to disambiguate from the replace buttons.
- **Assumption drafts stage-gate fix 2026-09-01:** `map-canvas.tsx` was passing `[]` to `AssumptionsRow.drafts` whenever `map.current_stage !== "assumptions"`. During honing (map has advanced past that stage) newly-regenerated drafts would persist to the DB but not render — coachee saw "N fresh drafts added above" with nothing above. Removed the stage gate; `AssumptionsRow`'s existing `isLocked` short-circuit still covers the never-hit case at earlier stages.
- **Replace-#N buttons show target-text preview 2026-09-01:** compact chip → two-line button. Top line is the `Replace #N` action label; bottom line is a truncated preview (60 chars) of the target assumption's text, so coachees can see WHICH assumption gets replaced without hovering for the tooltip. Small visual cost, clear intent match.
- **Remove worry with cascade 2026-09-01:** worries now have a `Remove` button (top-right of the row, next to `Needs more depth` if present). New server action `removeWorry` in `src/app/itc/actions.ts` + `deleteWorry` helper in `src/lib/itc/maps.ts`. Delete cascades via existing FKs: `itc_commitments (worry_id, ON DELETE CASCADE)` → paired commitment gone; `itc_assumption_commitments (commitment_id, ON DELETE CASCADE)` → coverage links gone. Assumptions themselves stay (coachee-authored, worth preserving); tests stay (attached to assumptions, not commitments). No test-blocking check needed because the assumption row itself isn't deleted — that's what differentiates this from `removeAssumption`, which IS test-blocked. Themed destructive confirmation names exactly what goes away. All three column reviews (worries, commitments, assumptions) invalidated on remove.
- **The worry drafter is deleted; Column 3 is his to write 2026-09-03:** the last path still authoring a fear was the REWRITE. It offered "if I told the client the honest, limited outcome, I'd be the guy selling confidence he doesn't actually have" against a behavior of over-promising: selling confidence he doesn't have IS over-promising, so it read "if I stopped overselling I'd be an overseller". The check that exists to catch that (`scoreWorryDepth` criterion 4, self-protection) was measured on six fresh cases and got four, missing only in the permissive direction. Pulling the question into its own isolated judge scored 0/4, and the main model failed it identically to the utility model, so the difficulty is in the QUESTION: answering "does this habit protect him from this fear, or cause it?" needs what is true inside one man's head, and both readings are constructible from the text every time. So Column 3 no longer gets a rewrite on either surface (`coachTextForWorry` and the hone path both return `suggestedFix: null`); the row shows the take, naming what is missing, and he writes the sentence. Deleted with it, ~500 lines: `draftWorryForBehavior`, `draftWorryOutcome`, `DRAFT_WORRY_SYSTEM`, `WorryDraftSchema`, `WorryDraftOutcome`, `assembleWorry`, `normalizeConditionalTense` + `PARTICIPLE_TO_BASE`, `WORRY_HARD_WORD_CAP`, `WorryIdentityShape` + `WORRY_IDENTITY_SHAPES` + `worryShapeInstruction` and their server-owned rotation, and `checkWorryLogicalConsistency` + its revealer/scaffolding tables in `rubric.ts` (it existed to catch inverted identity landings; there are none to invert). `coach.ts` 3850 → ~3340 lines. `draftWorryOpening` is untouched and still writes "I worry that if I <counter-move>, ". Regression `r`(e) now locks the ABSENCE, which is a stronger invariant than the agreement it used to assert.
- **One judge, one evidence set 2026-09-03:** `checkPeopleFromMap` refuses a draft that introduces a person the map never names. It was right; what it was shown was not. Every save-time caller in `fixes.ts` omitted `mapTexts` while every hone-time caller passed it, and `reviseBehavior` / `reviseAssumption` had no such parameter at all (`reviseBehavior` saw only the goal and the entry). So on a map whose WORRY names a wife but whose goal and behavior don't, the save-time rewrite was refused for adding "she" while the hone-time rewrite of the identical finding was accepted: no "Use this" on the row, a "Use this" in the banner. `mapTexts` is now REQUIRED on every drafter and reviser and on the five `coachTextFor*` helpers, so a blind call site is a compile error rather than a bug on someone's map; the compiler found ten, two of which a code review had missed. New `loadMapTexts()` sits immediately above `mapTextsOf()` and draws on the same five columns, with a test holding them together and another refusing `mapTexts: []`. Separately `words()` in `criteria/people.ts` tokenized "she's" whole, so `has("she")` was false and a phantom "she" shipped in an auto-derived commitment; each token now also yields the part before its apostrophe. The tests that looked like they covered this each had a bare "she" elsewhere in the same string and passed for the wrong reason.
- **Nothing is offered that the coach would flag on arrival 2026-09-03:** `draftCommitmentOutcome` knowingly returned a draft that failed its own verification, taking whichever attempt had fewer failures on the reasoning that the row carries its own sharpen box. A red box on words he did not write reads as the coach contradicting itself. Draft mode now offers a draft only when nothing in it is WRONG: depth 2/3 still ships (the badge already says "One more pass", and an invitation to go deeper is not a contradiction), while a phantom person, an interior-witness verb, or depth below 2 means no commitment is written and he writes his own. `commitments-row.tsx` now says so on the empty row rather than leaving a blank box beside two filled ones. Also: `DEPTH_THRESHOLD` was hardcoded as a bare `3` in nine comparisons in `coach.ts`, so moving the constant would have shifted the gate for the judges and silently left every drafter on the old bar.
- **Being seen by others is a worry, on every surface 2026-09-03:** the app held two positions. The intro read BEFORE writing said "not what they'd feel or how it'd look to someone else", while the rubric required a predicate naming who he'd be SEEN AS and the drafter's canonical shapes are "they'd see…" and "I'd be seen as…". He was told not to write the thing the app then demanded of him. The guides give both, joined by "or", twice in one passage (Vol 1 p 13): "How would doing the opposite damage the way you most like to SEE YOURSELF OR BE SEEN?", and their worked maps lean on the second half throughout. The intro now excludes the other person's FEELINGS, which was the real intent. With it, `scoreWorryDepth` criterion 2 gained an explicit scope: it judges WHOSE experience the worry is about and nothing else, because a thin identity was dragging it down too and the reason got written from the wrong criterion ("This is about what they'd think of you, not what you'd feel about yourself"), sending him toward the interior phrasing the voice rules ban. Vagueness and comparatives belong to criterion 3 alone. Verified the rubric still rejects practical concerns, the other person's feelings, external events, bare failure events, the behavior said back and third-person.
- **RE-DERIVE removed 2026-09-03:** its docstring said it "re-runs the cluster-first assumption drafter"; the body cleared assumption drafts, counted commitments and returned ok, never calling a drafter. Nothing updated the assumption's timestamp either, so the banner offering it reappeared immediately and the button visibly did nothing. Not repaired, because repairing it would put assumption drafting back through a side door. When a linked commitment moves, the assumption is his to re-read.
- **The assumption's "if" anchors on the vow, not the behavior 2026-09-03:** the coach offered "I assume that if I say the price and wait, then they'd see I'm not worth it…" against the worry "I worry that if I said the price and waited, they'd see I'm not worth what I'm charging". Same act, same dread, stem swapped. The model was following orders: `reviseAssumption`'s prompt said `antecedent_act` "must be the coachee doing the OPPOSITE of one of his behaviors", which IS the worry's if-clause, so the consequent had nowhere to go. The guides anchor it on the COMMITMENT BEING BROKEN every time ("not becoming my brother (Kurt)" → "if I am bragging"; "always feeling the freedom of having lots of options" → "if I don't have lots of options"). New deterministic `checkAssumptionRestatesWorry` backstops it in the rewrite's verification: 80% coverage threshold with light stemming, short worries skipped, since sharing the map's nouns is normal when both entries are about the same part of his life. Not fixed: `draftAssumptionsFromCommitments` carries the same behavior anchor and will produce the same restatement when `fixCoverage` fires.
- **The column review reports on the set; the rows do the coaching 2026-09-02:** a worry that needed work showed the coach's take and its rewrite in the row's box, and then the "Coach's take on this set" panel a few pixels below reprinted the same quote, the same line and the same rewrite verbatim. Both surfaces render the same `Finding[]`, so per-entry detail was appearing twice. Now that every column except the goal carries a coach box on each row, the review's job is the set: `renderFindings` takes `entriesCarryTheirOwnBox` and, in `column_review` mode, returns the opening plus a pointer ("Two things to work on in your worries before you move on. Each one is marked on its row above.") rather than the per-entry paragraphs. The goal column passes false, since it has no row box and its detail has nowhere else to live. One thing, said once, on the surface that owns it.
- **Every stage intro audited against the guides and the checks 2026-09-02:** the same pass applied to the other four columns, with the structural test extended so each bullet names a criterion the system actually runs. **Goal:** "Write a rough version in the box below and save it, I'll react" was interface narration AND a promise of an LLM goal reaction that was deleted when the criteria module took over that column. Bullets now cover all four of Appendix A's Column 1 criteria plus the bundled-goal check: "one improvement, not two", "**it implicates you**" (the criterion Vol 1 flags on its own worked map, where Jackson's goal was about what other people should do, and which had no bullet at all), "something you keep failing at", "it matters to you and belongs to this pillar". **Worries:** the self-protection criterion, the one the coach flags most often, had no bullet, so a coachee was being told about a bar only after he tripped it. Added: "The behavior protects you from it… If the behavior is what causes the thing you named, that's a price you pay, not the fear underneath." Dropped the now-redundant "start every one with 'I worry that if I…'" (the box opens with it) and degendered the examples. **Commitments:** "Under each worry, the coach has drafted a starting point. Review it, tap to use…" was stale (commitments have been auto-derived since 2026-08-31, there is no draft to accept) and used a banned UI verb; replaced with "The coach writes a first version of each one from your worry." Added a bullet for the interior-witness trap the regex flags ("Names who you'd be, not a feeling you'd dodge"). **Behaviors:** already mapped cleanly to its three rubric criteria; examples degendered.
- **Column 5 gets no prefilled stem at all 2026-09-02 (final):** the commitment-anchored opening was still wrong, for a reason the guides make plain. Their Big Assumptions take several shapes and many are not if-then ("I assume that saying anything about my accomplishments is bragging"; the Vol 1 p 19 quiz list includes "My self-worth is based on how others view me", "I don't deserve to feel happy"), and Vol 1 p 4 asks only that AT LEAST ONE be in if-then form. A stem picks one shape for him, and one built from his own commitment mostly hands back what he already wrote. So Column 5 offers nothing: `draftAssumptionOpening`, `buildAssumptionOpenings`, `draftAssumptionsAfterAdvance` and the assumptions "Rewrite openings" button are deleted, and `saveAssumption`'s existing `ensureStem` still writes the "I assume that" prefix on whatever he types. What he gets instead is what a coach would give him: the vows in front of him (`CompetingCommitmentsReference`, a read-only numbered box above Column 5 whose numbers match the link chips, server-rendered so an edit or a coach re-derive shows on the next render) and the guide's question under the box ("For one of those vows to feel that necessary, what has to be true?"). Coverage stays where the guide puts it (Vol 1 p 17), on honing's `assumption_uncovered_commitment` finding.
- **Column 5 follows Column 3: the opening, not the belief 2026-09-02:** the same split applied to Big Assumptions. `draftAssumptionOpening` returns only the opening and the server assembles `"I assume that if <the vow failing>, then "`; the coachee writes the Big Time Bad. **Corrected same day:** the first version anchored the "if" to the behavior's counter-move, and a live map showed why that is wrong. An opening built from a Column 2 counter-move leaves him nowhere to go but restate his Column 3 worry, and that is exactly what a coachee wrote. The guides' worked maps (Vol 2 pp 420-435, 660-685) always take the assumption's "if" from the COMMITMENT being violated: "I am committed to not becoming my brother Kurt" → "I assume that **if I am bragging**, I am just like Kurt"; "I am committed to always feeling the freedom of having lots of options" → "I assume that **if I don't have lots of options**, life will be bleak". Anchored to the broken vow, the only thing left to write is what it would cost him. The hint under the box carries the guide's own question for this move (Vol 1 p 17: for that vow to feel this necessary, what has to be true?). `buildAssumptionOpenings` generates one per commitment for COVERAGE, not because assumptions are 1:1 with commitments (the guide wants fewer, each underwriting several, Vol 1 p 3) — the link chips let one saved assumption cover several, and consuming an opening surfaces the next uncovered one. Shared by all three entry points (advance, regenerate, re-derive). `AddAssumptionForm` opens with the next pending opening and its commitment chip pre-selected; saving consumes it so the next surfaces, which walks him across the commitments that have no assumption yet. `DraftCard` and its "Use this draft" / "Dismiss" affordances are deleted, as is the whole-belief batch drafter's role in the product (`draftAssumptionsFromCommitments` remains only as the fallback used by honing's coverage finding). Live: "I assume that if I told the client a realistic scope and timeline, then " / "…if I asked the client what they actually needed, then " / "…if I named my price and pushed back on their pushback, then ", 3.0 s for a column.
- **The worry drafter writes the opening, the coachee writes the fear 2026-09-02:** the from-scratch worry drafter is retired. Across many live maps the counter-move it derived was right every time, while the identity ending was wrong roughly one time in three, and four rounds of sharpening the depth rubric's self-protection criterion moved it from about 70% to about 80% and stalled. Measured on the two behaviors that kept failing, four generated candidates each: 3/4 and 2/4 were good, and every one of the eight passed the judge, so verification was not the filter. That is the tell: the judge is the weak link and the man is not. Kegan/Lahey Vol 1 p 12 has the coach ASK "what are the worst things that could happen to you if you were to do the opposite?" and the client answer it; the app was answering for him, using the one input it can never have. New `draftWorryOpening` returns only the counter-move, the server assembles `"I worry that if I <counter-move>, "`, and `worries-row.tsx` opens the input with it: no draft card, no "Use this draft" button, nothing to accept, just a sentence to finish. Live: "I worry that if I told them the real scope and timeline, " / "…if I asked what they actually needed first, " / "…if I listened without bringing up her past, ", 5.5 s for a whole column. `draftWorryForBehavior` survives ONLY in `revise` mode, where it sharpens the fear the coachee wrote (a far easier and more legitimate job than inventing one) and feeds the "Sharper:" line in the row's coach box. Chips of 3-4 candidate endings were considered and rejected on usability: three behaviors times four options is a wall of choices at the moment he should be feeling one thing.
- **Score once, carry the verdict; and diagnostics for the assumption batch 2026-09-02:** a coachee accepted a coach draft and the row immediately red-boxed the same words. The draft was being scored TWICE by the same function: once by the drafter to decide whether to offer it, again by `saveWorry` on accept. Two calls to a model, so the two can differ (model tier is env-configured and can change, a deploy can land between drafting and accepting, and borderline cases sit near the line), and every difference reads as the coach contradicting a draft it just offered. The verdict now travels with the draft: `itc_behaviors.coach_worry_draft_depth_score` + `_rubric_reason` and the same pair on `itc_assumption_drafts` (migration `20260902000001_itc_draft_verdicts.sql`), written by `draftWorryOutcome`'s verification, reused by `saveWorry` when the saved text is byte-identical to the draft. Any edit falls through to a fresh score, because the verdict belongs to that exact text. One fewer model call on the accept path. Separately, `draftAssumptionsOutcome` now returns `{ drafts, refusals, uncoveredCommitmentIndices, error }` and `draftAssumptionsAfterAdvance` records an `llm_attempt` event with all of it, so "one draft for three commitments" is explainable from the admin view. The worry drafter has carried these diagnostics since 0689006; the assumption batch had none, which is the same blind spot that made the worry failures take four rounds to find.
- **Same two gaps swept into Column 5 2026-09-02:** after fixing the worry rubric, checked whether the same shapes of bug lived elsewhere. (a) The token/thinking middleware is global by construction: every `generateObject` / `generateText` call in `src/` goes through `mainModel()` / `utilityModel()`, so nothing else needed changing (verified by grep). (b) `scoreAssumptionDepth` had the worry rubric's old gap: a backwards Big Assumption ("if I hold firm on the terms, then they'd see me as desperate", when caving is what looks desperate) scored 3/3, identical to the correct version. It now carries the same extract-then-decide discriminator, `which_produces_the_consequent: "the act named in the if" | "the opposite of that act"`, folded into `lands_in_identity_or_big_time_bad`; the instruction reads the "then" as one chain (the act happens, it goes the way he dreads, then this) so an identity clause naming the other behavior isn't misread as backwards. 6/6 on the calibration set. (c) `checkAssumptionUnderwritesCommitments` skipped assumptions with fewer than two links, on the theory that one link can't be "the wrong one of the set"; but Appendix A criterion 1 asks of EVERY assumption whether believing it makes its commitment necessary, and the judge answers correctly for a set of one (verified). It now runs whenever there is at least one link. (d) `scoreCommitmentDepth` was checked and needs nothing: a noble commitment scores 0/3 on all three criteria, because the mirror-the-worry criterion already does the work. Draft-time and audit-time share these judges, so both paths are fixed at once; existing rows carry stored scores and need `itc:rescore-assumptions` to pick up the new bar.
- **Extended thinking off, output-budget floor on, in one middleware 2026-09-02:** the empty responses and the 40-80 second waits had one cause. Production runs `claude-sonnet-5`, which has extended thinking on by default; local ran a Haiku-class model, which does not, so every prompt in this codebase was written and tuned against a non-thinking model and the mismatch only showed in production. Measured with the worry drafter's real system prompt: thinking on = 25.4 s and 2132 output tokens; thinking off = 2.3 s and 53. The object itself is ~60 tokens. Deliberation consumed budgets sized for the answer (`maxOutputTokens: 200` / `512`), so calls returned nothing, and each blank burned a Kegan shape. `src/lib/model-config.ts` now wraps both tiers in one `wrapLanguageModel` middleware that sets `thinking: { type: "disabled" }` and floors `maxOutputTokens` at `MIN_OUTPUT_TOKEN_BUDGET` (4000). One place rather than 29 call sites: the number at a call site states the intent (how long the answer should be), the middleware guarantees the invariant (room to reach it), and a call site written later cannot opt out by forgetting. Test seams (`setMainModelOverride`) are returned unwrapped so a stub is exactly what the test installed. Turning thinking off is not a quality trade in this architecture: quality is enforced after generation by the depth rubrics and the criteria module, which re-score every draft and reject what fails. End to end, three worry drafts went from 80 s (with two empty) to 7.1 s (all three, no refusals). **Also fixed with it:** `scoreWorryDepth` criterion 4 was a single lenient boolean that passed a backwards worry ("if I held firm on the terms, they'd see me as desperate", when agreeing to bad terms is what looks desperate). Tuning that one dial strict rejected real fears and lenient passed contradictions, because it asked for a judgment call. Replaced with the criterion itself. Appendix A Column 3 ("commitment to self-protection; shows why Column 2 behaviors make good sense") is a RELATION WITH TWO DIRECTIONS, and every earlier version encoded only one of them, which is why each caught one failure shape and missed another: `behavior_prevents_the_feared_result` (while he keeps doing this, is he safe from that?) AND `opposite_brings_the_feared_result_about` (if he did the opposite, could this follow?). Both must hold. The one-direction version passed "if I asked the client what's really driving this, I'd be the one who sold them something they never needed" — a real fear, but NOT asking makes that more likely, not less, so it is not what the behavior protects him from and a commitment built on it explains nothing. 9/9 on the calibration set (three live drafts plus the six earlier cases). The worry word cap went 20 → 26 (target 15-22): 20 was forcing clipped, ungrammatical endings ("seen me as not enough alone", "not the expert I claim"), and Kegan-canonical worries run 15-25 words.
- **A failed model call is a blip, not a coaching verdict 2026-09-02:** with the fix above deployed, a map still drafted one worry out of three, and the new `llm_attempt` / `worry_draft` events named the cause exactly: zero refusals, and `errors: ["role_noun: No object generated: the model did not return a response", …]` on every shape. The same three behaviors drafted 3/3 locally, so it was environment, not logic. Each failed call was consuming a Kegan shape, so four blips left the coachee with nothing. New `generateWithRetry` retries the generation once on an error (400 ms backoff) and rethrows if it fails twice; a refusal by the CHECKS is never routed through it, since that is a real verdict about the text and has its own feedback retry. The worry and commitment drafters' `maxOutputTokens` went 200 → 600: they were the tightest budgets in the file for two-field schemas under very large system prompts, so any preamble or reasoning before the object hit the cap and returned nothing. The `llm_attempt` event and the drafter's error string now carry `mainModelIdOrUnset()`, so a recurrence names the model tier it happened on.
- **Missing worry drafts: one judge for identity, four shapes, outcomes in turn events 2026-09-01:** a fresh sales map got one draft out of three. Turn events showed the advance-time drafts finishing in 10 s, i.e. refusals, not timeouts; local reproduction found the deterministic "layer 3" identity-marker whitelist in `checkWorryLogicalConsistency` refusing "they'd have seen me as someone who doesn't belong in this room", a canonical seen-as landing the rubric scores 3/3. Two judges for one question, disagreeing. Layer 3 and `WORRY_IDENTITY_MARKERS` are deleted; whether a landing reaches the identity rung is `scoreWorryDepth.touches_identity`'s call alone (framing and interior-scaffolding stay deterministic). `draftWorryOutcome` returns `{ text, refusals[], error? }` (`draftWorryForBehavior` is the text-only wrapper); `draftMissingWorriesAfterAdvance` walks all four Kegan shapes from the behavior's rotation slot until one is verified and records an `llm_attempt` event per behavior with `shapes_tried`, every refused draft with the checks' lines, and any error, so an empty draft slot is explainable from `/itc/admin` instead of a server log.
- **Worry drafts: both Kegan shapes, never the behavior said back, and nothing unverified reaches the card 2026-09-01:** on a sales map the drafter offered "if I asked what they actually needed first, I've been the guy who never listens to anybody" and "if I told a client exactly what I believe will work, I've been choosing my reputation over serving them." Confessions of the behavior, not fears of the opposite; under the direct fear-box → Column 3 route they become vows to stop the behavior. Three structural causes, three fixes: (1) the drafter prompt taught only the EXPOSURE shape ("what the counter-move would REVEAL"), which on a work map degenerates into the behavior restated. It now names what the behavior PROTECTS him from in either canonical shape, exposure (something the behavior hides) or consequence (who he'd be if the opposite went badly, Vol 1 p 13 "seen as incompetent"), with the self-test "under this fear, does the behavior make sense as protection?"; the role-noun shape's example verbs are no longer listening verbs. (1b) `scoreWorryDepth` gained a fourth criterion, Appendix A Column 3 "shows why Column 2 behaviors make good sense", as an EXTRACT-THEN-DECIDE block: the judge writes `opposite_move` and `feared_result_of_opposite`, then rules `feared_result_is_the_behavior_restated` and `behavior_protects_him_from_this`; `explains_behavior` is derived from those and folded into `touches_identity` so the score stays 0–3. Asked as one inline boolean the judge rationalized both confessions as fears; with the two lines written first it rejects both (0/3) and keeps the real one (3/3), on the utility model. The advance-time drafter falls back to the next Kegan shape once when a shape's draft is refused. `checkWorryLogicalConsistency`'s identity marker accepts any "the [role] who…", so work roles count. (2) `checkWorryLogicalConsistency` accepted only revealer framing ("I've been…"), sending consequence drafts back toward confession; it now accepts "I'd be / I'd be seen as / I'd have proven", with the behavior-said-back inversion caught semantically by `scoreWorryDepth`. (3) The worry drafter and the assumption batch drafter returned their best attempt even when it failed verification ("never silent drop"). They now offer only what passed; the batch gives each uncovered commitment one targeted attempt, and what still fails is left to the coachee and to honing's coverage draft. Structural tests lock all three.
- **People come from the map 2026-09-01:** on an Amplify map that names no one ("what the other person sees"), the worry drafter offered "she'd see I've never actually cared what she thinks." The "she" came from the drafter's server-owned shape instructions (`"she'd see I've been the [husband|…] who…"`) and its marriage-scene examples; the voice doc's rule that every pronoun trace to the coachee's text lived only in prompts. New deterministic `checkPeopleFromMap` (`criteria/people.ts`): a gendered pronoun or relational noun in a draft must appear in the map's own text (goal + entries + paired context), else the draft is refused with feedback naming who the map does have (or "they / the other person" when it names no one). Runs in every drafter, reviser and suggester verify loop; a structural test locks that. `witnessFromMap` picks she'd / he'd / they'd from the map and `worryShapeInstruction` builds the four Kegan shapes against that witness, so no example carries a wife. Each drafter prompt also gets a "People this map names: …" line. Coachee-typed text is not held to this; it's his map.
- **Every coaching surface verified by the judge that scores it on save 2026-09-01 (the sweep):** a "Give me ideas" chip ("I shut down and withdraw rather than stay present…") drew a red box the moment the coachee tapped it: `generateSuggestions` never ran `scoreBehaviorDepth`. Swept every path by which text reaches the map. Rule: anything the system offers as text the coachee can accept must already have passed the function that scores it when he does. Same function, not a parallel prompt. (1) `generateSuggestions` runs `verifySuggestion` per chip (behavior → `scoreBehaviorDepth`, goal → `hasCompetingGoalFraming`, worry / commitment / assumption → their rubrics), keeps chips at 3/3, one retry with the rejected chips and the rubric's reasons as feedback, and every chip and the intro line go through `scrubBannedCoachWords` (the same chips had "my read of the situation"). (2) `draftAssumptionsFromCommitments` adds `scoreAssumptionDepth` to its per-draft verification; it had only the deterministic checks, so a draft could land at 2/3 and get a red box on tap. (3) Behaviors join the shared surface: `itc_behaviors.sharpen_text` + `suggested_fix` (migration `20260901000002_itc_behaviors_coach_text.sql`), new `reviseBehavior` verified by `scoreBehaviorDepth`, `coachTextForBehavior` in fixes.ts, `attachFixes` persists onto behavior rows, `CoachFixBox` with "Use this" on the row. (4) Structural tests in `units.test.ts`: each suggester / drafter / reviser block must reference its judge; the stage intros' "What makes a good…" bullets must name the same criteria the rubric scores. (5) Deleted the dead per-entry LLM reaction (`generateCoachReaction`, `fireCoachReaction`, `buildReactionPrompt`, opener rotation) and chat reply (`generateCoachChat`); `ReactionOutput` survives as the plain shape `persistReaction` writes for "Give me ideas". See "How coaching text is verified" below.
- **Worry rubric rejects the behavior said back as self-criticism 2026-09-01:** a coachee sharpened a worry to "I worry that if I let her past rest and listened, I've been the man who brings up her mistakes to dodge mine" (behavior: "I bring up things she did in the past instead of listening"). The map takes the guide's direct route from fear box to Column 3 (Vol 1 p 13: fear baked in, "I'm also committed to never [the feared identity]"), so the auto-derived commitment came out as a vow to stop the behavior, a noble commitment, exactly the leak p 13 warns about. Under the direct transform the commitment is precisely as self-protective as the worry's identity, so the fix is on the worry side only: `scoreWorryDepth.touches_identity` now strictly rejects an identity that restates the paired Column 2 behavior (the behavior text was already in its prompt) and requires the fear to name what the behavior protects him from. The worry drafter's verify loop reads the same rubric, so its rewrites carry the bar. Existing worries re-scored with `itc:rescore-worries`. No commitment-side criterion added: checking a mechanical transform's output for a property only its input can have would be a second judge for one question.
- **One identity bar across columns; depth severity mirrors the gate 2026-09-01:** on the same real map, A2 ("then I have failed at the thing I claim to be great at") and A3 ("then it will expose that I may not be as capable or valuable as they believe I am") scored 3/3 on `scoreAssumptionDepth` while `scoreWorryDepth` strictly rejects those exact shapes (bare failure event; comparative hedge). Two rubrics, opposite verdicts on the same words. The assumption rubric's `lands_in_identity_or_big_time_bad` now carries the worry rubric's strict-reject list (comparative hedges, bare failure events, outcomes about others) and the same "finish 'the identity he's afraid of being is: ___' from the text itself" self-check. Separately, depth-shortfall findings were always `critical` ("broken"); now `depthSeverity(score)` in `criteria/types.ts` mirrors `worryPassesDepth`: 2/3 is moderate ("one thing to fix"), below 2 is critical. Applied on all four columns.
- **Enactability moved to where the guide asks it; rewrites held to the underwrite + identity bar 2026-09-01 (follow-up):** a Kegan/Lahey-voiced review of the rebuilt audit on a real map found it flagging the two ROOT assumptions (the broad beliefs that hold up all four commitments) as "not enactable" and offering rewrites that turned them into duplicates of their children. Vol 1 p 21 is explicit that the most powerful Big Assumptions arrive untestable in draft form; p 4 asks the question at the map level ("are there testable assumptions, or do they all have such dire outcomes…"); p 18 raises the bar only for the one he tests. So: (1) at hone / column review, `checkAssumptionsHaveAnEnactableIf` fires only when NO assumption on the column has an enactable "if" (then every one gets the finding); the per-assumption judge runs at **prioritize** on the selected assumption (`coachTextForSelectedAssumption`, called from `selectAssumptionForTesting` and the pre-select in `deliverPrioritizeRecommendationAfterAdvance`), writing a behavior-anchored rewrite onto that row. Save-time assumption feedback is row-level only (depth + vague-then), no judges. (2) `reviseAssumption` verifies every rewrite with `judgeAssumptionUnderwrites` against the assumption's own linked commitments, so a rewrite that drifts into another commitment's room is refused, not flagged later. (3) New deterministic `checkAssumptionKeepsCommitmentIdentity` (criteria/assumptions.ts): the drafted/rewritten "then" must carry a role noun extracted from the linked commitments' canonical "the [role] who…" form (or a self-label like "fraud"); fails open when nothing is extractable. Applied in both the fresh batch drafter and rewrite mode (one bar). Catches the observed drift ("the man who's been faking it" under coach/father commitments).
- **Hone audit + inline coach rebuilt on the guide's own Column 4 criteria 2026-09-01:** the audit on a well-clustered map was producing eight findings, all from two checks the Kegan/Lahey guides never ask for (`assumption_commitment_drift`, one LLM call per link, and `assumption_overload`, one per assumption, both prompt-biased to fire). The drafter was told to cluster one assumption under several commitments (the guide's target); the auditor punished exactly that. Both checks are deleted. In their place: (1) `assumption_doesnt_underwrite`, Appendix A criterion 1 ("makes the Column 3 commitment absolutely necessary"), one call per assumption, biased to keep, and its judge `judgeAssumptionUnderwrites` is the SAME function the drafter's `verifyDraftClusters` uses, so a cluster the drafter accepts can't be rejected on hone; (2) `assumption_not_enactable`, Checkpoint 2 (Vol 1 p 18, "the 'if' has degrees so it can be enacted safely"), the guide's actual honing criterion for Column 4, which the old audit never checked. `assumption_uncovered_commitment` downgraded to moderate (p 17: start from the juiciest commitments, not cover every one). **Every finding now carries a verified rewrite.** New `src/lib/itc/fixes.ts` runs the drafters in rewrite mode (`draftWorryForBehavior` / `draftCommitmentForWorry` gained `revise`; new `reviseAssumption`) against the finding's own lines and keeps the result only if it clears the same checks; `render.ts` prints it as `Sharper: "…"` and the row shows a "Use this" button (`coach-fix-box.tsx`). **Volume budget:** the hone banner prints ONE entry (Vol 1 p 5, "think first about what most needs to be sharpened"), the column review three; the rest are counted ("Three more after this. They're marked on the map.") and every affected row carries its own box. No more "(a)/(b)" sub-lists: an entry's findings render as one paragraph with one rewrite. **One voice:** `sharpen_text` + `suggested_fix` columns on `itc_worries` / `itc_commitments` / `itc_assumptions` (migration `20260901000001_itc_sharpen_text_and_suggested_fix.sql`, self-contained: also re-declares the 0831 staleness columns/functions); `rubric_reason` restored to the raw rubric reason; column review and hone write their findings onto the rows, so the box under an entry, the review and the banner are the same text. Staleness triggers retargeted to fire on content changes only (text / selected / links), so the audit writing coach text can't mark itself stale. `ADVICE` rewritten in coach voice (second person, contractions, no dashes, no machinery or interface words); new voice test in `advice-alignment.test.ts` and `render.test.ts` locks it. `row-sharpen.ts` now returns `Finding[]` (`rowFindingsFor*`); `composeXSharpen` deleted. `scripts/rescore-itc-worries.ts` writes the new triple. New `judges.test.ts` pins the two LLM checks' contracts with the model mocked.
- **Coach-advice single source of truth 2026-09-01:** new `src/lib/itc/criteria/advice.ts` module — `ADVICE: Record<IssueType, string>` — is the one place every coach surface reads its fix-suggestion text from. Retires the drift-prone triangle where three files (render.ts CRITIQUE_SPECS + each criteria/*.ts `detail` string + coach.ts drafter retry feedback) each hand-wrote parallel copies of the same advice and could silently diverge. The 2026-09-01 bug that motivated this: `interior_witness_worry` advice said "flip to 'she'd see...'" while `scoreWorryDepth.is_first_person_felt` explicitly rejects centering on her reaction — coachee did what the audit said, then the save-time rubric rejected the rewrite. Two coaches, contradictory advice. Fix: (a) all check-function `detail` strings now read from `ADVICE[issueType]` (depth findings still append the LLM's dynamic `rubric_reason` for granular why-it-failed context, but the fix-direction sentence is the same across every surface). (b) `render.ts` `CRITIQUE_SPECS` = programmatic map from `GENERIC_CRITIQUE_ISSUE_TYPES` → `ADVICE[t]`; local strings deleted. (c) `coach.ts` drafter retry-loops for interior-witness and mirrors-worry-identity read from `ADVICE`. (d) `pickExternalPronounForWorries` deleted (dead code after removing "she'd see" suggestion). New structural test `advice-alignment.test.ts` locks three invariants: every `IssueType` has an `ADVICE` entry (exhaustiveness); `ADVICE.interior_witness_worry` includes a first-person rewrite pattern and any "she'd see/say" mention is in an anti-pattern warning frame (not a recommendation); six protected files (worries/commitments/assumptions/behaviors/render/coach) must not re-introduce the retired "Flip to outside witness" recommendation outside `ADVICE`. Loosening this test to make it pass defeats the guard — edit `ADVICE` instead.
...
- `draftAssumptionsFromCommitments` — batch-drafts 2-6 assumptions as slot sets (antecedent + tell + identity). Server assembles each with `"I assume that if I …, then … and …"` structure. The tell/identity join is shape-aware (see `buildIdentityPredicate` in `src/lib/itc/coach.ts`): a bare-noun identity gets an injected "I'd be" (or bare "be" when the tell already carries an I-subject) so any well-formed slot pair produces a grammatical sentence, not just the cases where the identity happened to attach cleanly to the tell's final verb. Assembled sentences run through `trimAssembledDraft`, which is guaranteed to return a string (soft 20-word target, never silent-drop). Deterministic consistency check on each draft; if any fail, one batch retry fires with per-draft feedback. **Post-retry cluster-fit verifier** (`verifyDraftClusters` inside the same function): for any draft with 2+ commitment_indices, an LLM utility call runs the Kegan/Lahey cluster-coverage test (does the antecedent_act actually violate each linked commitment's protective vow?) and drops non-fitting links before the drafts reach the coachee's Draft card. Same architectural family as `checkAssumptionOverload` (which runs on saved assumptions during honing) — preventative rather than reactive. Verifier never strips a cluster to zero and falls back to the original cluster on error, so a wrong verdict is never worse than no verdict.
...
Column 4 (commitments) is the load-bearing column; a sharp end-of-column review here has outsized impact on the assumptions and tests that follow. Column 5 (assumptions) review checks the guide's own Column 4 criteria: does believing the assumption make each linked commitment necessary, and is the "if" a move he can make.

### How coaching text is verified (one rule, every surface)

Anything the system offers the coachee as text he can accept has already passed the function that scores it when he does. Same function, not a parallel prompt. Judges live in `src/lib/itc/rubric.ts` (depth rubrics), `src/lib/itc/rules.ts` (goal framing), and `src/lib/itc/criteria/` (row and cross-entry checks, incl. the two LLM judges on Column 5).

| Path | Verified by |
|---|---|
| "Give me ideas" chips | `verifySuggestion` → the kind's rubric (or `hasCompetingGoalFraming` for goals); voice scrub |
| Worry drafter (`draftWorryForBehavior`) | `scoreWorryDepth` + consistency + interior-witness regex |
| Commitment auto-derive (`draftCommitmentForWorry`) | `scoreCommitmentDepth` (+ mirror bool) + interior-witness regex |
| Assumption batch drafts (`draftAssumptionsFromCommitments`) | `scoreAssumptionDepth` + consistency + `checkAssumptionKeepsCommitmentIdentity` + `judgeAssumptionUnderwrites` |
| Rewrites (`fixes.ts` → `reviseBehavior` / worry + commitment drafters in `revise` mode / `reviseAssumption`) | the same checks as the row, returned only at 3/3 (assumptions also re-judged for underwrite and enactability when relevant) |
| Save-time box, column review, hone banner | criteria module (`runColumnCriteria` / `runHoneWaterfall`) → `attachFixes` → `renderFindings` / `renderRowSharpen`; findings persisted onto rows |
| Prioritize pre-select and selection | `judgeAssumptionEnactable` on the selected assumption (`coachTextForSelectedAssumption`) |
| Test design draft | `reviewTestDesign` (SMART), the same review that gates "Run the test" |
| Stage intros | static; structural test ties each "What makes a good…" bullet to a rubric criterion |

Retired: per-entry LLM reactions (`generateCoachReaction`) and chat replies (`generateCoachChat`). Their prompts had their own idea of "deep" and disagreed with the rubrics; the deterministic renderer over the criteria module replaced them.
...
The audit synthesis path is **pure deterministic template rendering, no LLM in the render step** (two checks on the assumptions column fire narrow structured LLM judges; the rewrites come from the drafters). Uses the shared criteria module (`src/lib/itc/criteria/`), the same checks that power end-of-column reviews and the row box. The pipeline (2026-09-01):

1. **`runHoneWaterfall`** (`src/lib/itc/criteria/orchestrator.ts`) walks the map top-down (`goal → behaviors → worries → commitments → assumptions`) via `runColumnCriteria` and stops at the first column with findings. Returns `{ column, findings }`. Waterfall rationale: everything downstream of a broken column will be re-derived once the coachee fixes it, so critiquing them now is asking to fix something about to change. Matches the derivation chain the Kegan/Lahey guide describes (Vol 1 p 4).
2. **`attachFixes`** (`src/lib/itc/fixes.ts`) drafts a verified rewrite for every entry that has a drafter (worry / commitment / assumption; a coverage gap gets a Big Assumption draft on the draft card instead) and writes `sharpen_text` + `suggested_fix` onto the rows so the box under each entry says what the banner says. Reuses the row's stored rewrite when the finding set is row-level only. `persist: false` for the admin preview.
3. **`renderFindings`** (`src/lib/itc/criteria/render.ts`) with `mode: "hone"` prints the top entry only (`limit` defaults: hone 1, column review 3; admin preview passes `Infinity`): `"quote"` / what's-off line(s) / `Sharper: "…"`, then "N more after this. They're marked on the map." Openings count every entry shown or marked: "Your Amplify map holds up. One thing to work on before you pick what to test." or, when something blocks the gate, "Your Amplify map is close. Three things to work on before you pick what to test. Start with the first two; the map under them moves once they're finished." Appreciative voice: "holds up" / "is close", never "broken" (2026-09-01).

Priority (`prioritizeEntries`): broken (critical) entries before fixable ones, then top of the map before bottom, then map order. Findings on one entry render as one paragraph with one quote and one rewrite; there is no sub-list.

Issue types the checks emit: `bundled_goal`, `depth_shortfall_behavior`, `interior_witness_worry`, `interior_witness_commitment`, `commitment_doesnt_mirror_worry`, `vague_assumption_then_clause`, `depth_shortfall_worry` / `_commitment` / `_assumption`, `assumption_doesnt_underwrite` (Appendix A criterion 1; one LLM call per multi-link assumption via `judgeAssumptionUnderwrites`, shared with the drafter's cluster verifier; fix is a link change, rendered as "Drop #2 from it"), `assumption_not_enactable` (Checkpoint 2, Vol 1 p 18; `judgeAssumptionEnactable`, also run at save time on the row), `assumption_uncovered_commitment` (moderate), `test_coverage_gap`, `test_grip_through_data`. Retired 2026-09-01: `assumption_commitment_drift` and `assumption_overload` (not in the guide; fired per link on any well-clustered map, i.e. on exactly the maps the drafter is told to produce).

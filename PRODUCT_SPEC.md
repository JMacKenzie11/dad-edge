...
3. **`renderFindings`** (`src/lib/itc/criteria/render.ts`) with `mode: "hone"` prints the top entry only (`limit` defaults: hone 1, column review 3; admin preview passes `Infinity`): `"quote"` / what's-off line(s) / `Sharper: "…"`, then "N more after this. They're marked on the map." Openings: "Your Amplify map holds up. One thing to fix before you pick what to test." or "…has one thing that's broken. Fix it first."

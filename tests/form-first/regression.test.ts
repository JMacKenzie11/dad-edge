...
describe("Form-First regression", () => {
  ...
  it(
    ...
    async () => {
      ...
      for (const msg of chatTurns) {
        ...
        expect(res.ok, `sendCoachMessage failed: ${res.ok ? "" : res.reason}`).toBe(true);
      }
  ...
  it(
    "regression 5: coach service down still lets a full map get built",
    async () => {
      const model = makeHardFailModel();
      setMainModelOverride(model);
      setUtilityModelOverride(model);
      ...
      // Verify goal + behaviors on map despite LLM being down.
      const { data: mapRow } = await supabase
        .from("itc_maps")
